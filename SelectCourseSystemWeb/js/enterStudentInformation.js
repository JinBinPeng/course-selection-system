...
function Ajax1() {
    $.ajax({
        ...
        cache:false,
        data:new FormData($('#uploadform')[0]),   // 通过这样来传表单文件
...
function Ajax2() {
    $.ajax({
        type:"post",
        url:"http://139.199.220.49:8888/api/user/excel",
        dataType:"json",
        async:false,      // 设置为同步，成功调用时的值可以传到ajax函数外面
        cache:false,
...
setTimeout(function Ajax3() {
    $.ajax({
        type: "get",
        url: "http://139.199.220.49:8888/api/user?pageSize=8",
        dataType: "json",
        async: false,      // 设置为同步，成功调用时的值可以传到ajax函数外面
        cache: false,
        contentType: "application/json",
        ...
        success: function (data) {
            ...
            $.each(table, function (index, item) {
                if (index > 0) {
                    $("#stutab").html($("#stutab").html() + "<tr align=\"center\">" + "<td>" + item.department + "</td>" +
                        "<td>" + item.studentNumber + "</td>" + "<td>" + item.name + "</td>" +
                        "<td>" + item.sex + "</td>" + "<td>" + item.phoneNumber + "</td>" + "</tr>");
                }
            });
        }
    });
